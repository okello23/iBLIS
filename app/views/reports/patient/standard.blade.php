...
<table style="border-bottom: 1px solid #cecfd5;">
	<tr>
		<td colspan="5"><strong>{{ trans('messages.patient-name')}}</strong></td>
		@if(Entrust::can('view_names'))
			<td colspan="8">{{ $patient->name }}</td>
		@else
			<td colspan="7">N/A</td>
		@endif
		<td colspan="4"><strong>{{ trans('messages.gender')}}</strong></td>
		<td colspan="3">{{ $patient->getGender(false) }}</td>
		<td colspan="2"><strong>{{ trans('messages.age')}}</strong></td>
		<td colspan="3">{{ $patient->getAge()}}</td>
		<td colspan="3"><strong>{{ trans('Lab ID')}}</strong></td>
		<td colspan="8">{{ $patient->ulin}}</td>
	</tr>
</table>
<table style="border-bottom: 1px solid #cecfd5;">
	<tr>
