...
// GROUP BY t.specimen_id";
	...
	private function getTestCountsByLabSection($lab_section_id, $month){
		$query = "select ut.test_type_id, tt.name, utr.measure_id, tt.test_category_id as lab_section, count(ut.id) as total, SUM(case when ut.interpretation = 'Positive' then 1 else 0 end) as Positive FROM unhls_tests ut INNER JOIN test_types tt ON(ut.test_type_id = tt.id) INNER JOIN test_categories tc ON(tc.id = tt.test_category_id) INNER JOIN unhls_test_results utr ON(utr.test_id = ut.id) INNER JOIN testtype_measures ttm ON(ttm.measure_id = utr.measure_id)
